...
응답 201: `{ "bookmark": { …전체 필드, "tags": ["React", "프론트엔드"] } }`. `tags`는 최대 5개의 문자열 배열이다. 단축어는 이 응답의 `bookmark.title`을 알림 표시에 활용 가능.

### GET /api/bookmarks — 목록

쿼리: `categoryId` (uuid | `none`=미분류 | 생략=전체), `q` (title/url/description/tags 부분 검색), `cursor`, `limit` (기본 30, 최대 100). 태그 클릭 검색도 별도 쿼리 없이 기존 `q`를 사용한다.
...
- `PATCH /api/bookmarks/:id` — `title`, `description`, `categoryId`(null 허용=미분류로), `url`, `tags` 수정. `tags`는 공백을 trim하고 중복을 제거하며 최대 5개, 각 20자 이하다. 카테고리 수동 변경 시 서버가 `ai_status='idle'`로 되돌림
...
- `PUT /api/ai/keys/:provider` `{ apiKey }` → 해당 provider 키만 암호화 저장/교체하며 활성 모델은 변경하지 않음
- `PUT /api/ai/model` `{ provider, model }` → 키가 등록된 provider의 고정 카탈로그 모델을 활성 모델로 저장. 키가 없거나 provider/model이 불일치하면 400
