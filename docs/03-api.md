...
- `Content-Type: multipart/form-data`, 필드 이름은 `item`, 요청당 링크 텍스트 또는 이미지 파일 하나를 받는다.
...
3. `각 항목 반복` 안에서 `POST https://<서버주소>/api/share` 호출
4. 헤더 `X-API-Key: bm_…`, 본문은 폼 필드 `item=<현재 반복 항목>`
5. `Content-Type`은 직접 지정하지 않고 단축어가 multipart boundary를 생성하게 함
