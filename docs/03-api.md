...
응답 201: `{ "bookmark": { …전체 필드, "kind": "link", "url": "https://…", "image": null, "tags": ["React", "프론트엔드"] } }`. `tags`는 최대 5개의 문자열 배열이다. `aiModel`은 AI가 분류에 실사용한 OpenRouter 모델 id(raw, 미분류/미시도면 `null`)다. 단축어는 이 응답의 `bookmark.title`을 알림 표시에 활용 가능.
...
- `PATCH /api/bookmarks/:id` — `title`, `description`, `categoryId`(null 허용=미분류로), `url`, `tags` 수정. `url` 변경은 링크 항목에서만 허용한다. `tags`는 공백을 trim하고 중복을 제거하며 최대 5개, 각 20자 이하다. 사용자가 이 필드 중 하나라도 수정하면 서버가 `ai_status='idle'`로 되돌려 pending AI 결과가 수동 편집을 덮어쓰지 못하게 한다.
