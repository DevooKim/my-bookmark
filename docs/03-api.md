...
응답 201: `{ "bookmark": { …전체 필드, "tags": ["React", "프론트엔드"] } }`. `tags`는 최대 5개의 문자열 배열이다. `aiModel`은 AI가 분류에 실사용한 OpenRouter 모델 id(raw, 미분류/미시도면 `null`)다. 단축어는 이 응답의 `bookmark.title`을 알림 표시에 활용 가능.
