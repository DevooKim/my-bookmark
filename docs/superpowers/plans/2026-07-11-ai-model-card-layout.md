...
- [x] Add a failing test that the `사용 모델` heading precedes `AI API 키` and its parent card has rounded/border classes.
- [x] Run the focused test and verify failure.
- [x] Move the existing model block above key cards and apply `rounded-xl border border-zinc-200 p-4 dark:border-zinc-800`.
...
- [x] Record the layout refinement and verification.
- [x] Run `pnpm typecheck && pnpm lint && pnpm test && pnpm build`.
- [x] Rebuild Docker, browser-check order/card styling, mark plan complete, and commit.
