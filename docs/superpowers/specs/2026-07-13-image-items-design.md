...
이미지 카드는 메뉴와 태그 검색 버튼을 제외한 카드 본문 전체가 앱 내부 상세 화면으로 이동하는 주 링크다. 썸네일이나 제목을 눌러도 같은 상세 화면을 열어야 한다. 상세 화면은 기본적으로 signed 썸네일을 viewport 높이의 약 70%를 쓰는 큰 미리보기 영역에 표시한다. 사용자가 `원본 보기`를 눌러야 원본 이미지로 전환하고 원본 다운로드 액션을 노출하며, `미리보기로 돌아가기`로 다시 썸네일을 볼 수 있다. 미리보기와 원본 URL 오류는 소스를 전환할 때 각각 한 번씩 갱신을 시도한다. 제목, 요약, 태그, 카테고리, AI 모델, 재분석, 삭제는 두 모드에서 동일하게 제공한다. 이미지 리마인더 알림은 외부 URL 대신 상세 화면을 연다.
