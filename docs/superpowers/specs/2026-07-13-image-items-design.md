...
이미지 카드는 메뉴와 태그 검색 버튼을 제외한 카드 본문 전체가 앱 내부 상세 화면으로 이동하는 주 링크다. 썸네일이나 제목을 눌러도 같은 상세 화면을 열어야 한다. 상세 화면은 viewport 높이의 약 70%를 쓰는 큰 원본 이미지 영역과 제목, 요약, 태그, 카테고리, AI 모델, 다운로드, 재분석, 삭제를 제공한다. 이미지 리마인더 알림은 외부 URL 대신 상세 화면을 연다.
