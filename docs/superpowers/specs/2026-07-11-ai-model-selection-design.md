...
- API 키가 없는 provider의 모델은 disabled이며 `API 키 필요`를 표시
...
- 키가 없는 provider 모델은 UI에서 선택을 막고 서버에서도 거부한다.
...
- 모델 optgroup, 키 없는 provider 옵션 disabled, 모델 저장
...
원격 마이그레이션 적용 후 Docker 스택과 실제 설정 화면에서 모델 선택 저장, 저장된 OpenAI 키 연결 테스트, 키 미설정 provider 옵션 비활성화를 확인한다.
