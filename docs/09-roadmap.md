...
2. 각 Phase의 수용 기준을 전부 만족하고 `bun run typecheck && bun run lint && bun run test && bun run build`가 통과해야 완료다.
...
- [ ] 루트 `package.json`의 Bun workspaces(apps/*, packages/*), `bun.lock`, 루트 스크립트: `dev`(web+api 병렬), `typecheck`, `lint`, `test`, `build`
- [ ] `tsconfig.base.json` (strict, ES2023 target). 각 패키지가 extends
- [ ] Biome 설정 (`biome.json`) — lint+format, 루트 1개
- [ ] `apps/web`: **공식 CLI로 스캐폴딩** (`bun create @tanstack/start`) 후 이 저장소 구조로 이식. Tailwind v4 설정. 스캐폴드가 만든 예제 라우트는 홈 placeholder로 정리
...
수용 기준:
- [ ] `bun install && bun run dev` → web(:3000) 렌더, `curl localhost:3001/api/health` → `{"ok":true}`
...
- [ ] api: push 라우트(구독 upsert/해지/test), `services/push-sender.ts` (410 정리 포함), 단발·매일·매주·매월 reminders 라우트, `services/reminder-cron.ts` (조건부 클레임 — 06 순서도 그대로), graceful shutdown
- [ ] web: `lib/push.ts` 구독 플로우(사용자 제스처, iOS 미설치 안내), 설정 알림 섹션(토글+테스트 버튼), 지난 이력·다시 알림·반복 활성 토글을 포함한 리마인더 페이지와 설정 다이얼로그, 알림 꺼짐 배너
...
- [ ] 단발은 발송 후 status=sent와 빨간 날짜로 목록에 남고 다시 알림 가능. 반복은 다음 회차로 이동하며 비활성화 중에는 발송 안 됨. 휴지통으로 취소한 항목만 목록에서 사라짐
...
고급 반복 규칙(rrule·종료일·횟수), 북마크 가져오기/내보내기, 전문 검색(FTS), 브라우저 확장, 읽기 모드 아카이브.
